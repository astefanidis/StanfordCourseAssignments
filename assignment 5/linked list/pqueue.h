...
private:
   ...
   void deepCopy(const PriorityQueue & src) {
        if(src.list == NULL){
            ...
            listElement *copyHead = new listElement;
            copyHead->value = src.list->value;
            copyHead -> priority = src.list->priority;
            listElement *i = new listElement;
            listElement *j = new listElement;
            j=copyHead;
            for(i = src.list; i->next!=NULL; i=i->next){
                listElement *nextElem = i->next;
                listElement *newElem = new listElement;
                newElem->value = nextElem->value;
                newElem->priority = nextElem->priority;
                j->next = newElem;
                j=j->next;
            }
            list = copyHead;
